...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        inputs = self.processor(img, sentence, max_length=40, padding='max_length', truncation=True, return_tensors='pt')
...
def calculate(model, loader, infer=False):
    ...
    with torch.no_grad():
        for _, data in enumerate(tk):
            ...
            pixel_values = torch.stack(list(pixel_values)).squeeze(1).to(device)
            pixel_mask = torch.stack(list(pixel_mask)).squeeze(1).to(device)
            vecs = model(input_ids=ids, attention_mask=att_mask, pixel_mask=pixel_mask, pixel_values=pixel_values).pooler_output
...
if __name__ == '__main__':
    with open('image_vilt.pkl', 'rb') as f:
